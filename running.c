...
void calculate_pairwise_forces() {
    ...
    for (i = 0; i < global.N_particles; i++)
        for (j = i + 1; j < global.N_particles; j++) {
            ...
            if (r2 < 16.0) {
                // use r2/dt as lookUpTable index
                int lookUpTableIndex = r2 / global.dt;
                // if the r2 is already calculated, extract f/r from the look-up array, else
                // compute the value r2, f/r2 values and add them in the look-up arrays
                if (global.r2_values[lookUpTableIndex] != 0.00) {
                    if (r < 0.1) {
                        f = 100.0;
                    } else {
                        f = global.f_over_r_values[lookUpTableIndex];
                    }
                } else {
                    r = sqrt(r2);
                    ...
                    f = f / r;
                    global.r2_values[lookUpTableIndex] = r2;
                    global.f_over_r_values[lookUpTableIndex] = f;
                }
